...
describe BeersController do
  ...
  def mock_beer(stubs={})
    @mock_beer ||= mock_model(Beer, stubs)
  end

  describe "GET index" do
    it "assigns all beers as @beers" do
      Beer.stub(:find).with(:all).and_return([mock_beer])
      get :index
      assigns[:beers].should == [mock_beer]
    end
  end

  describe "GET show" do
    it "assigns the requested beer as @beer" do
      Beer.stub(:find).with("37").and_return(mock_beer)
      get :show, :id => "37"
      assigns[:beer].should equal(mock_beer)
    end
  end

  describe "GET new" do
    it "assigns a new beer as @beer" do
      Beer.stub(:new).and_return(mock_beer)
      get :new
      assigns[:beer].should equal(mock_beer)
    end
  end
  ...
  describe "POST create" do
    ...
    describe "with valid params" do
      it "assigns a newly created beer as @beer" do
        Beer.stub(:new).with({'these' => 'params'}).and_return(mock_beer(:save => true))
        post :create, :beer => {:these => 'params'}
        assigns[:beer].should equal(mock_beer)
      end

      it "redirects to the created beer" do
        Beer.stub(:new).and_return(mock_beer(:save => true))
        post :create, :beer => {}
    ...
    describe "with invalid params" do
      it "assigns a newly created but unsaved beer as @beer" do
        Beer.stub(:new).with({'these' => 'params'}).and_return(mock_beer(:save => false))
        post :create, :beer => {:these => 'params'}
        assigns[:beer].should equal(mock_beer)
      end

      it "re-renders the 'new' template" do
        Beer.stub(:new).and_return(mock_beer(:save => false))
        post :create, :beer => {}
  ...
  describe "PUT update" do
    ...
    describe "with valid params" do
      it "updates the requested beer" do
        Beer.should_receive(:find).with("37").and_return(mock_beer)
        mock_beer.should_receive(:update_attributes).with({'these' => 'params'})
        put :update, :id => "37", :beer => {:these => 'params'}
      end

      it "assigns the requested beer as @beer" do
        Beer.stub(:find).and_return(mock_beer(:update_attributes => true))
        put :update, :id => "1"
        assigns[:beer].should equal(mock_beer)
      end
    ...
    describe "with invalid params" do
      ...
      it "assigns the beer as @beer" do
        Beer.stub(:find).and_return(mock_beer(:update_attributes => false))
        put :update, :id => "1"
        assigns[:beer].should equal(mock_beer)
      end
  ...
  describe "DELETE destroy" do
    ...
    it "redirects to the beers list" do
      Beer.stub(:find).and_return(mock_beer(:destroy => true))
      delete :destroy, :id => "1"
